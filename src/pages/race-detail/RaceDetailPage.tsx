...
export function RaceDetailPage() {
  ...
  const [openRouteMapRaceId, setOpenRouteMapRaceId] = useState<string | null>(null)
  ...
  const routeMapOpen = Boolean(openRouteMapRaceId && race && openRouteMapRaceId === race.id)
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") setOpenRouteMapRaceId(null)
    }
    ...
    <div className="min-h-screen bg-background text-foreground">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
                  <div className="flex w-full shrink-0 flex-col gap-1.5 sm:w-[7.25rem]">
                    {routePreviewInteractive ? (
                      <button
                        type="button"
                        onClick={() => setOpenRouteMapRaceId(race.id)}
      ...
      {routeMapOpen && race.hasRoute && routeModalSrc ? (
        <div
          ...
          role="presentation"
          onClick={() => setOpenRouteMapRaceId(null)}
        >
          <div
            role="dialog"
            aria-modal="true"
