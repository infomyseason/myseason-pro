...
export function RaceDetailPage() {
  ...
  const [routeMapRaceId, setRouteMapRaceId] = useState<string | null>(null)
  const routeMapOpen = routeMapRaceId === raceId
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") setRouteMapRaceId(null)
    }
    ...
    <div className="min-h-screen bg-background text-foreground">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
                  <div className="flex w-full shrink-0 flex-col gap-1.5 sm:w-[7.25rem]">
                    {routePreviewInteractive ? (
                      <button
                        type="button"
                        onClick={() => setRouteMapRaceId(race.id)}
      ...
      {routeMapOpen && race.hasRoute && routeModalSrc ? (
        <div
          ...
          role="presentation"
          onClick={() => setRouteMapRaceId(null)}
        >
          <div
            role="dialog"
            aria-modal="true"
